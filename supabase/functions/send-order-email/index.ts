...
// Supabase Edge Function that sends order confirmation emails.
// Uses Resend HTTP API (works on Supabase Edge / Deno Deploy where raw SMTP is blocked).
// Set RESEND_API_KEY as a Supabase secret. Free tier: 100 emails/day.
// Optionally set EMAIL_FROM (e.g. "QuickBites <orders@yourdomain.com>").
// If you don't have a custom domain verified on Resend yet, use "onboarding@resend.dev".
...
// Resend HTTP API config (set these as Supabase secrets)
const RESEND_API_KEY = Deno.env.get('RESEND_API_KEY') || '';
const EMAIL_FROM = Deno.env.get('EMAIL_FROM') || 'QuickBites <onboarding@resend.dev>';
...
serve(async (req) => {
  ...
  try {
    ...
    // ----- Parse body (defensive: avoid InvalidContentType from req.json() on some runtimes) -----
    ...
    try {
      if (contentType.includes('application/json')) {
        ...
        return jsonResponse({ error: `Invalid Content-Type. Expected application/json, got: ${contentType}` }, 400);
      }
    } catch (parseErr) {
    ...
    if (!RESEND_API_KEY) {
      console.error('RESEND_API_KEY not set. Add it as a Supabase secret.');
      return jsonResponse({ error: 'Email service not configured — missing RESEND_API_KEY' }, 500);
    }
    ...
    const subject = `Order Confirmed — #${emailData.orderCode} from ${emailData.restaurantName}`;

    // ----- Send via Resend HTTP API -----
    console.log(`Sending email to ${recipientEmail} via Resend...`);
    ...
    if (!resendRes.ok) {
      ...
      return jsonResponse({ error: resendBody?.message || `Resend returned ${resendRes.status}` }, 500);
    }

    console.log(`Order confirmation email sent to ${recipientEmail} for order ${orderId}`, resendBody);
